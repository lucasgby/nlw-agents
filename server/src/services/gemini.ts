...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  ...
  if (!response.text) {
    throw new Error('Não foi possível convert o áudio.');
  }
...
export async function generateAnswer(
  ...
  const prompt = `
    ...
    INSTRUÇÕES:
     ...
     - Se a resposta não for eencontrada no contexto, apenas responda que não possue informações sucifientes para responder;
     - Seja objetivo;
